...
out-of-tree plugin: the host loads `typio_engine_sherpa.so` at runtime and
feeds it microphone audio, and the engine returns recognized text.
...
This installs the plugin and its icons. The host discovers it on next start —
